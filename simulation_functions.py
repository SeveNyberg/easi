...
def change_frame(v, mu, u):
    ...
    Parameters: 
        ...
        u (numpy array (float)): The velocity of the new frame relative to the old one. 
...
def eval_rg(M_A, theta_1, beta):
    """
    Compute gas compression ratio of the shock iteratively with Newton's method. Borrowed from Alexandr Afanasiev's SOLPACS plotting code.
...
def eval_rb(M_A, rg, theta_1):
    """
    Compute the magnetic compression ratio of the shock.
...
def eval_box_size(mfp_ambient, ux1, theta_1, v):
    """
    Compute the upstream box size of the simulation based on the focusing length derived from the diffusion coefficient along the shock normal.
    ...
    Parameters:
        ...
        v (float): The speed of the particle for which the box size will be set. Note that slower particles will be in DSA regime and faster particles will not.
        
    Returns:
        L (float): The upstream box size of the simulation.
    """
...
def eval_E_natural(E, m_e, c, e):
    ...
    Returns:
        E (float): Energy in the natural units used in the simulation
    """
...
def write_init():
    """
    This function creates a directory in which to store the results of 
    the simulation and returns the path to the directory relative to 
    the simulation script's location.
    
    Returns:
        (string): The path to the run folder relative to the simulation script.
    """

    # Create a path to store all the results of all simulation runs
...
def save_prm(path):
    ...
    # Create a dict to store atrtibutes and values in
    p = {}
    ...
    # Delete the imported from the name space to not interfere with the simulation code
    del prms
