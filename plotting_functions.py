...
def load_all_data(run_nos, snapshot):
    ...
    Parameters:
        ...
        snapshot (int): An integer indicating the chosen snapshot. Use 0 as snapshotting has not been re-tested.
    ...
    return particles, inthist, pinthist, paths, inj_specs, prms
    
    
def plot_espec(particles_list, parameters_list, paths, 
               xlims = [None, None], ylims = [None, None], 
               scframe = True, upstream = True, downstream = True, theory = True,
               titles = None, savefigs = False, show = True, color_start = None):
    '''
    ...
    Parameters:
        ...
        scframe (boolean): Toggle whether to plot in the upstream plasma frame or the shock frame, True for plasma frame.
...
def plot_hist(inthist_list, parameters_list, paths, 
    ...
    for title_i, (inthist, p, path) in enumerate(zip(inthist_list, parameters_list, paths)):
        ...
        box_size = eval_box_size(p["L_ambient"] * 1000, p["ux1"] * 1000, np.deg2rad(p["theta_1"]), v_L) / ion_inert
        ...
        # Else use the default equidistant grid
        else:
            x_grid = np.linspace(x_min, x_max, p["x_N"])
            x_edges = np.linspace(x_min, x_max + (x_max-x_min)/p["x_N"], p["x_N"]+1)
        ...
        # Pot the upstream and downstream portions of the histogram separately per function inputs
        if upstream:
...
def plot_2dhist(inthist_list, parameters_list, paths, 
    ...
    for inthist, p, path in zip(inthist_list, parameters_list, paths):
        ...
        # If using custom spatial bins, evaluate the grid
        if p["custom_bins"]:
            x_edges = p["spatial_bins"]()
            x_grid = x_edges[:-1]
        # Else use the default equidistant grid
...
def plot_2dmomhist(pinthist_list, parameters_list, paths, 
    ...
    for pinthist, p, path in zip(pinthist_list, parameters_list, paths):
        ...
        # If using a custom spatial grid, evaluate it
        if p["custom_bins"]:
            x_edges = p["spatial_bins"]()
            x_grid = x_edges[:-1]
        # Else use the default equidistant grid
        ...
        if units_E:
            ax[0,0].set_ylabel(r"$E_{\parallel ,\mathrm{f}}$ [keV]", fontsize = fonts)
        else:
        ...
        if units_E:
            ax[1,0].set_ylabel(r"$E_{\parallel ,\mathrm{b}}$ [keV]", fontsize = fonts)
        else:
        ...
        for loc in arr_loc:
            arrcolor = next(citer)
            xloc = loc
            xloc0 = xloc - (xmax-xmin)/100
            xloc1 = xloc + (xmax-xmin)/100

            xarr = [xloc0,xloc1,xloc,xloc0]
            yarr = [ymin, ymin, ymin*0.8, ymin]
            
            ax[1,0].plot(xarr,yarr,color=arrcolor)#, lw = 3, ms = 3)
            ax[1,0].vlines(xloc, ymin = ymin, ymax = ymax, color = arrcolor)
        ax[1,0].invert_yaxis()
        ...
        if units_E:
            ax[0,1].set_ylabel(r"$E_\perp$ [keV]", fontsize = fonts)
        else:
        ...
        if units_E:
            x_vals =  p_to_E(p_grid[:-1])*m_e*c**2/e/1000
        else:
            x_vals = p_grid[:-1]
        ...
        if units_E:
            ...
            ax[1,1].set_xlabel(r"$E_{\parallel ,\mathrm{b}}$ [keV]", fontsize = fonts)
        else:
            ax[1,1].set_xlim(xlims)
...
def plot_profiles(parameters_list, path_list, zoomin = False, 
    ...
    for ind, (parameters, path) in enumerate(zip(parameters_list, path_list)):
        ...
        inv_L_ambient = 1/(p["L_ambient"] * 1000 / ion_inert)        
        ...
        if ind == 0:
            ...
            plt.plot(x, np.rad2deg(np.arctan(tan_theta)))
        ...
        if legend:
            plt.legend(loc = (0.1,-1.0))#, transform = plt.gca().transAxes)
